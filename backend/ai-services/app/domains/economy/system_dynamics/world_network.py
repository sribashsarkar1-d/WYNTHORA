...
class WorldNetwork:
    ...
    def _initialize_nations(self):
        ...
        try:
            ...
            engine = create_engine(settings.sync_database_uri, connect_args={'connect_timeout': 2})
    ...
    async def tick(self, state_store: Any) -> None:
        """
        Advances the global network by one tick asynchronously.
        Reads/Writes from the global state store.
        """
        # Fetch any macro context needed
        ...
        # Run CPU-bound nation ticks in a thread pool so it doesn't block the async event loop
        loop = asyncio.get_running_loop()
        tasks = [
            loop.run_in_executor(None, process_nation, n) 
            for n in self.nations.values()
        ]
        await asyncio.gather(*tasks)
            
        # Simplified Trade Impact
        if self.trade_matrix is not None:
            self.update_trade_network()
            gdp_array = np.array([n.gdp for n in self.nations.values()])
            
            exports_matrix = self.trade_matrix * gdp_array
            self.exports = np.sum(exports_matrix, axis=0)
            self.imports = np.sum(exports_matrix, axis=1)
    ...
    async def get_global_state(self) -> Dict[str, Any]:
        """
        Deprecated. Use state_store.get_all() instead.
